...
has "History", "General Nonfiction," and "Fiction" categories that I now try to read from. 

My goal is to have read at least one of the finalists from each Pulitzer year (and to see how far back I can go too).
...
bar_colors <- c("#7a5195", "#ef5675", "#1a9988")
...
plot_table <- pullitzer %>%
  ...
  mutate(across(c(read,author.of.color,author.female), ~ifelse(.x == 1,'Yes','No')),
         ...
         category = recode(category, 'history' = 'History','general-nonfiction'='General Non-Fiction', 'fiction' = 'Fiction')) %>%
